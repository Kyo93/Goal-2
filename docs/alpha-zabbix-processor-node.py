...
def main(zabbix_export_file=None, zabbix_export_body=None):
    if zabbix_export_body is not None and _clean_text(zabbix_export_body) != "":
        if isinstance(zabbix_export_body, (bytes, bytearray)):
            body_text = bytes(zabbix_export_body).decode("utf-8-sig", errors="replace")
        else:
            body_text = str(zabbix_export_body)
            if body_text.startswith("\ufeff"):
                body_text = body_text.lstrip("\ufeff")
        if _looks_like_html(body_text):
            return _build_error("HTTP body looks like HTML/login page; expected CSV export", "zabbix_csv")
        reader = csv.reader(io.StringIO(body_text), delimiter=",", quotechar='"')
        return _process_rows(rows=reader, source_type="zabbix_csv", source_label="zabbix_csv")

    if zabbix_export_file:
        try:
            path = Path(_resolve_file_path(zabbix_export_file))
            if not path.exists():
            ...
            if path.exists():
                ...
                rows, source_label = _rows_from_xlsx(path)
                return _process_rows(rows=rows, source_type="zabbix_xlsx", source_label=source_label)
        except Exception as exc:
            return _build_error(
                ...
                rejected=[
                    {
                        "row_number": None,
                        "source_ref": "zabbix_export_file",
                        "reason": f"Failed to read Zabbix export: {exc}",
                        "row_data": {"zabbix_export_file": str(zabbix_export_file)},
                    }
                ],
            )
        ...
        rejected=[
            {
                ...
                "row_data": {
                    "zabbix_export_file": str(zabbix_export_file) if zabbix_export_file is not None else None,
                    "zabbix_export_body_type": type(zabbix_export_body).__name__ if zabbix_export_body is not None else None,
                },
            }
        ],
    )
